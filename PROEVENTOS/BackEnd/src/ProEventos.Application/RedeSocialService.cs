...
namespace ProEventos.Application
{
    public class RedeSocialService : IRedeSocialService
    {
        ...
        public async Task AddRedeSocial(int id, RedeSocialDto model, bool isEvento)
        {
            try
            {
                RedeSocial redeSocial = _mapper.Map<RedeSocial>(model);
                if (isEvento)
                {
                ...
                await _redeSocialPersist.SaveChangesAsysnc();
            }
            catch (Exception ex)
            {
        ...
        public async Task<RedeSocialDto[]> SaveByEvento(int eventoId, RedeSocialDto[] models)
        {
            try
            {
                ...
                var redeSocialsRetorno = await _redeSocialPersist.GetAllByEventoIdAsync(eventoId);

                return _mapper.Map<RedeSocialDto[]>(redeSocialsRetorno);
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        public async Task<RedeSocialDto[]> SaveByPalestrante(int palestranteId, RedeSocialDto[] models)
        {
            try
            {
                ...
                var redeSocialsRetorno = await _redeSocialPersist.GetAllByPalestranteIdAsync(palestranteId);
        ...
        public async Task<bool> DeleteByEvento(int eventoId, int redeSocialId)
        {
            try
            {
                ...
                if (redeSocial == null)
                    throw new Exception("Rede Social por evento não encontrado");

                _redeSocialPersist.Delete<RedeSocial>(redeSocial);
                return await _redeSocialPersist.SaveChangesAsysnc();
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        public async Task<bool> DeleteByPalestrante(int palestranteId, int redeSocialId)
        {
            try
            {
                ...
                if (redeSocial == null)
                    throw new Exception("Rede Social por palestrante não encontrado");
        ...
        public async Task<RedeSocialDto[]> GetAllByEventoIdAsync(int eventoId)
        {
            try
            {
                RedeSocial[] redeSociais = await _redeSocialPersist.GetAllByEventoIdAsync(eventoId);
                if (redeSociais == null)
                    return null;

                RedeSocialDto[] resultado = _mapper.Map<RedeSocialDto[]>(redeSociais);
        ...
        public async Task<RedeSocialDto[]> GetAllByPalestranteIdAsync(int palestranteId)
        {
            try
            {
                RedeSocial[] redeSociais = await _redeSocialPersist.GetAllByPalestranteIdAsync(palestranteId);
                if (redeSociais == null)
                    return null;
        ...
        public async Task<RedeSocialDto> GetRedeSocialEventoByIdsAsync(int eventoId, int redeSocialId)
        {
            try
            {
                RedeSocial redeSocial = await _redeSocialPersist.GetRedeSocialEventoByIdsAsync(eventoId, redeSocialId);
                if (redeSocial == null)
                    return null;

                RedeSocialDto resultado = _mapper.Map<RedeSocialDto>(redeSocial);
        ...
        public async Task<RedeSocialDto> GetRedeSocialPalestranteByIdsAsync(int eventoId, int redeSocialId)
        {
            try
            {
                RedeSocial redeSocial = await _redeSocialPersist.GetRedeSocialPalestranteByIdsAsync(eventoId, redeSocialId);
                if (redeSocial == null)
                    return null;
